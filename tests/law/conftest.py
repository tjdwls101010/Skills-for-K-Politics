...
`Scripts/` 를 import 경로에 넣어 `law` 패키지를 찾는다(PEP 723 인라인 의존성 + `uv run` 방식이라 pyproject 가 없다).

⚠️ **law 패키지에 고유한 이름을 쓰는 이유가 여기 있다.** `tests/congress/conftest.py` 가
공용 `Scripts/` 를 같은 `sys.path` 에 이미 올려 두므로, 양쪽이 `net.py`·`db.py` 를
