...
class Test다음_실행이_이어받는가:
    ...
    @staticmethod
    def _파싱(몸통, 목록행):
        return {"판례일련번호": 몸통["판례정보일련번호"], "사건번호": "2020다1",
...
class Test옛_표지를_되돌린다:
    ...
    ⚠️ **걷는 자리는 "실제로 다시 묻는다"에 결박된다.** A1 이 `본문 + 없음 = 목록` 을
    보므로, 이번에 안 물을 것의 표지를 걷으면 그 자료는 본문도 '없음'도 없는 상태가 되고
    **이번 실행이 고칠 수 없는 방향으로** 등식이 깨진다.
    """
    ...
    def test_4xx_에서_온_것만_고른다(self) -> None:
        ...
        저장.record_failure(conn, "판례", "C", "네트워크", "ReadTimeout")
    ...
    def test_표본_밖의_표지는_안_걷는다(self, monkeypatch) -> None:
        ...
        **이번 실행으로는 고칠 수 없게** 깨진다. 워크플로가 `inputs.sample` 을 실제로
        ...
        assert 본문 + len(남은표지) == len(목록), (
            f"A1 이 깨졌다: 본문 {본문} + 없음 {len(남은표지)} != 목록 {len(목록)}")
...
class Test장애가_남기는_것:
    ...
    아니라 `'네트워크'` 로 남고, 그러면 **A3 가 그 실행에서 빨갛다.** 타임아웃·5xx 가
    이미 그렇게 동작하며, 다음 실행이 받으면 풀린다 — 고칠 수 없는 빨간불이 아니다.

    반대쪽을 골랐다면(4xx 를 계속 '없음'으로 두면) A3 는 초록인데 자료는 영영 안 온다.
    **초록인 채로 잃는 것보다 빨간 채로 남는 쪽**이다.
    """

    def test_끝내_못_받으면_네트워크로_남는다(self, monkeypatch) -> None:
        monkeypatch.setattr(원천.time, "sleep", lambda *_: None)
        ...
        assert conn.execute(
            "SELECT 실패종류 FROM 수집실패 WHERE 자료ID='700'").fetchone()[0] == "네트워크"
