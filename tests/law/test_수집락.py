...
class Test수집_시각_기록:
    """`신선도.적재기준시각`(= `수집상태('전체','수집')`)은 **"우리가 마지막으로 원천을
    확인한 때"**다.
    ...
    **수집기가 며칠째 멈춰 있어도 "방금 확인함"으로 보인다.** 그게 이 값을 남기기로 한
    이유(원천은 우리가 언제 받았는지 모른다) 자체를 무의미하게 만든다.
    """

    def test_재파싱은_적재기준시각을_건드리지_않는다(self, tmp_path):
        db = tmp_path / "재파싱.db"
        conn = 연결.connect(db)
        이행.init_schema(conn)
        conn.execute(
            "INSERT INTO 수집상태 (자료종류,단계,상태,갱신일시,상태시작일시)"
            " VALUES ('전체','수집','완료','2020-01-01 00:00:00','2020-01-01 00:00:00')")
        conn.close()
        ...
        conn = 연결.connect(db)
        assert conn.execute(
            "SELECT 적재기준시각 FROM 신선도").fetchone()[0] == "2020-01-01 00:00:00"
        conn.close()
