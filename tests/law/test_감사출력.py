...
def test_각_게이트_줄이_이름과_값과_신호등을_함께_준다(conn, 출력) -> None:
    ...
    conn.execute(
        ...
        " VALUES ('M1','L1','법','법률','현행','x'), ('M2','L1','법','법률','현행','x')")
    conn.commit()
    conn.close()
    _, 글 = 출력()
    줄 = _게이트줄(글, "A11")
    assert "정리후보로 설명되지 않는 현행 중복" in 줄
    assert " 1  🔴" in 줄, f"값이나 신호등이 사라졌다: {줄!r}"
    assert _게이트줄(글, "A18").rstrip().endswith("0  🟢")


def test_빈_DB_는_빨갛고_마지막_줄이_몇_건인지_말한다(conn, 출력) -> None:
    """빈 DB 에서는 A1b(목록 단계를 한 번도 완료 못 한 자료종류)가 6 이다."""
    conn.close()
    코드, 글 = 출력()
    assert 코드 == 1
...
def test_빨간_게이트에는_해석이_따라_붙는다(conn, 출력) -> None:
    ...
    코드, 글 = 출력()
    assert 코드 == 1
    assert "🔴" in _게이트줄(글, "A11")
    assert any(줄.strip().startswith("A11:") for 줄 in 글.splitlines()), \
        "빨간 게이트의 해석이 사라졌다"
