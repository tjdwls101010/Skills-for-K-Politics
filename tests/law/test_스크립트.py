...
⚠️ **이 파일이 없어서 `직결.py` 의 문법 오류가 393개 테스트를 전부 통과했다.**
어떤 테스트도 그 모듈을 import 하지 않았기 때문이다. `직결.py` 는 사람이 손으로 부르는
...
파일들 = sorted(p.name for p in SCRIPTS.glob("*.py"))


def test_스크립트를_하나도_빠뜨리지_않았다():
    """새 스크립트가 생기면 이 목록이 자동으로 늘어난다 — 손으로 적지 않는다."""
    assert set(파일들) == {
        "감사.py", "검증.py", "수집.py", "적재.py", "직결.py",
    }
...
@pytest.mark.parametrize("이름", ["감사.py", "적재.py", "직결.py"])
