...
`검증.py`(법제처가 바뀌었나) · `pytest`(우리 코드가 회귀했나) 와 함께 진단 삼각형을 이룬다. 셋이 각각 다른 질문에 답하므로, 수집이 빈손으로 올 때 어느 쪽이 깨졌는지가 갈린다.
...
게이트: list[tuple[str, str, str]] = [
    (
        ...
        #    상수→DB 갱신은 `수집.py` 가 실행 중 `migrate()` 를 부르는 것이 전부인데,
...
보고: list[tuple[str, str, str]] = [
    ...
    #    맞는지도 모른다. D31 이 `해석기관명` 을 지우면서 그 감시를 `검증.py` V20 으로
    #    옮긴 것과 같은 수법이다 — **안 읽히는 컬럼을 감시되는 컬럼으로 바꾼다.**
    # ⚠️ 여기서 자료마다 날짜가 크게 벌어지는 것이 정상이다. 지금은 **증분 갱신이 없어서**
    #    이미 있는 행을 다시 받지 않는다(`lsHstInf&regDt` 가 `검증.py` V8 에만 있고 수집
