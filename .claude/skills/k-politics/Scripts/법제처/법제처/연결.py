...
SKILL_DIR = Path(__file__).resolve().parents[3]   # 법제처/법제처 → 법제처 → Scripts → k-politics
...
# ── 락 ──────────────────────────────────────────────────────
#
# ⚠️ **DB 를 고치는 경로는 전부 이걸 거쳐야 한다.** `수집.py` 만 잠그던 시절에는
#    `법령.py`·`판례.py` 같은 단독 실행이 락 밖에서 `이행()` 을 불렀고, 예약 수집과
