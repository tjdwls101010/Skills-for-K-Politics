...
주석패치: tuple[tuple[str, str, str], ...] = (
    ...
    ("국회",
     ...
     "    공포번호            TEXT,  -- **법령 쪽으로 건너는 다리다** — `법령.공포번호` 와 같은 번호이고, 이름이 갈린 경우에도 이어진다(의안 '12·29여객기참사피해구제및지원등을위한특별법' ↔ 법령 '12·29여객기참사 피해구제 및 지원 등을 위한 특별법'). 공포번호가 있는 의안 1,481건 중 922건이 이어진다.\n"
     "    -- ⚠️ **이 다리로 '이 법안이 고친 법'을 정하지 마라.** `법령.공포번호` 는 그 판본을 만든 개정의 번호라, 일괄개정 하나가 여러 법에 같은 번호를 남긴다(1,464개 번호 중 168개가 여러 법령ID를 가리킨다). 대상 법령은 `법률안대상법령` 이 이름으로 맞춘 것이고, 공포번호를 1순위로 넣어 본 실측에서는 7건이 붙는 대신 9건이 엉뚱한 법을 가리켰다.\n"
     "    -- NULL = 미발생 또는 원천값 없음."),
    ("국회",
