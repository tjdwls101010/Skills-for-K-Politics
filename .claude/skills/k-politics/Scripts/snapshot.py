...
주석패치: tuple[tuple[str, str, str], ...] = (
    ...
    # ── 합치면서 생긴 다리. 두 코퍼스를 한 파일에 넣은 이유가 이것인데 원천 주석은 서로를 모른다.
    ("국회",
     "    공포번호            TEXT,  -- NULL = 미발생 또는 원천값 없음.",
     "    공포번호            TEXT,  -- **법령 쪽으로 건너는 다리다** — `법령.공포번호` 와 같은 번호이고, 이름이 갈린 경우에도 이어진다(의안 '12·29여객기참사피해구제및지원등을위한특별법' ↔ 법령 '12·29여객기참사 피해구제 및 지원 등을 위한 특별법'). 얼마나 이어지는지는 SELECT COUNT(DISTINCT b.의안번호) FROM 의안 b JOIN 법령 l ON l.공포번호=b.공포번호 AND l.법종구분='법률'.\n"
     "    -- ⚠️ **이 다리로 '이 법안이 고친 법'을 정하지 마라.** `법령.공포번호` 는 그 판본을 만든 개정의 번호라, **일괄개정 하나가 여러 법에 같은 번호를 남긴다** — 한 공포번호가 몇 개 법을 가리키는지는 SELECT 공포번호, COUNT(DISTINCT 법령ID) c FROM 법령 WHERE 법종구분='법률' GROUP BY 1 HAVING c>1. 대상 법령은 `법률안대상법령` 이 이름으로 맞춘 것이고, 공포번호를 1순위로 넣어 본 실측에서는 새로 붙는 것보다 엉뚱한 법을 가리키는 것이 더 많았다.\n"
     "    -- NULL = 미발생 또는 원천값 없음."),
    ("국회",
     "    소관위원회          TEXT REFERENCES 위원회(위원회명),  -- ⚠️ 현재 소관이며 회부 전이면 NULL — 원천이 한 벌만 주므로 위원회 이관 이력은 담지 않는다.",
     "    소관위원회          TEXT REFERENCES 위원회(위원회명),  -- ⚠️ 현재 소관이며 회부 전이면 NULL — 원천이 한 벌만 주므로 위원회 이관 이력은 담지 않는다.\n"
     "    -- ⚠️ `소관기관.상임위` 와는 40종 중 16종만 겹친다(나머지는 소위원회와 개편 전 이름) — INNER JOIN 하면 그 24종의 의안이 조용히 빠진다."),
    ("법령",
     "  공포번호     TEXT,  -- 앞자리 0을 포함한 원천 표기가 판례의 '법률 제N호로 개정' 인용과 연결되므로 문자열로 보존한다. NULL은 미제공·빈 값이다.",
     "  공포번호     TEXT,  -- 앞자리 0을 포함한 원천 표기가 판례의 '법률 제N호로 개정' 인용과 연결되므로 문자열로 보존한다. `의안.공포번호` 와도 같은 번호다(그쪽 주석에 다리와 함정이 있다). NULL은 미제공·빈 값이다."),
    ("법령",
     "  소관부처     TEXT,  -- NULL은 원천 미제공·빈 값이다.",
     "  소관부처     TEXT,  -- `소관기관.소관기관` 과 **정확일치로** 잇는다 — 73종 중 64종이 이걸로 잡히고, LIKE 가 필요한 것은 괄호 표기 둘뿐이다(`소관기관` 주석 참조). NULL은 원천 미제공·빈 값이다."),
    # ── 법령
    ("법령",
...
CREATE TABLE 신선도 (
  ...
  감사통과     INTEGER,  -- 1 = 그 감사가 게이트를 다 통과. NULL = 감사 기록 없음. **0 = 위반이 남아 있었다** — 이 코퍼스를 근거로 답할 때는 `감사상세` 의 게이트가 무엇을 재는지 확인하거나, 못 하면 그 한계를 답에 밝힌다. 조용히 넘어가면 검증에 실패한 자료를 검증된 것처럼 내놓게 된다.
  감사시각     TEXT,     -- ⚠️ **그 감사가 언제 것인가.** 감사는 수집 파이프라인이 돌 때만 원장에 기록되므로, 수집이 며칠 멈춰 있었으면 이 시각도 그만큼 옛것이고 `감사통과=0` 은 **그때** 실패였다는 뜻이지 지금도 실패라는 뜻이 아니다. `적재기준시각` 과 벌어져 있으면 그 사이의 적재는 감사를 안 거쳤다. NULL = 기록 없음.
...
CREATE TABLE 법률안대상법령 (
  ...
  -- ⚠️ **미매칭(법령ID NULL) ≠ 그런 법이 없다.** 제정안(고칠 법이 애초에 없다)이 대부분이고, 통칭·구법명, 개정으로 법 이름 자체가 바뀌는 경우, "○○법 등 8개 법률" 같은 일괄개정도 여기 들어온다. 얼마나 되는지는 SELECT 매칭근거, COUNT(*) FROM 법률안대상법령 GROUP BY 1.
...
def _신선도(대상: sqlite3.Connection, 빌드시각: str) -> None:
    ...
    감사 = 대상.execute(
        "SELECT 상태, 건수, 상세, 갱신일시 FROM 국회.수집상태 WHERE 대상='감사'"
    ...
    대상.execute("INSERT INTO 신선도 VALUES ('국회',?,?,?,?,?,?)",
                 (시각, 통과, 감사시각, 상세, 경고, 빌드시각))
...
def build(원천국회: Path, 원천법령: Path, 소관기관: Path, 출력: Path,
    ...
    with _락획득(락들, 락타임아웃):
        ...
        try:
            ...
            _신선도(c, 빌드시각)
            c.execute(
                "INSERT INTO 신선도 VALUES ('소관기관',NULL,NULL,NULL,NULL,"
