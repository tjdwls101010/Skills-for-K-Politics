...
def _백업모듈():
    """`백업.py` 를 **파일 하나만 이름 붙여** 불러온다.

    ⚠️ **레포 루트를 `sys.path` 에 올리지 마라** — 네 스킬의 `Scripts/` 에 같은 이름이
    여럿 있어, 먼저 import 된 쪽이 다른 스킬을 조용히 깨뜨린다(테스트에서 실측했다).
    """
    if "백업" in sys.modules:
        return sys.modules["백업"]
    경로 = Path(__file__).resolve().parents[2] / "Scripts" / "백업.py"
    if not 경로.is_file():
        return None
    명세 = importlib.util.spec_from_file_location("백업", 경로)
    sys.modules["백업"] = m = importlib.util.module_from_spec(명세)
