...
def _모듈(스크립트: Path):
    ...
    if 스크립트 not in _모듈캐시:
        ...
        sys.path.insert(0, str(스크립트.parent.parent))
        ...
        이름 = f"backup_{스크립트.parent.name}_{스크립트.stem}"
...
코퍼스: dict[str, 명세] = {
    "law": 명세(
        이름="LAW",
        스크립트=스킬 / "Scripts/law/load.py",
        감사스크립트="Scripts/law/audit.py",
        # ⚠️ **"행이 하나라도 있나"를 여기서 묻는다.** 사본이 quick_check 를 통과해도
        #    내용이 비었으면 백업이 아니다. 진짜 하한(전회 대비 감소율)은 아래 `_비교` 가 진다.
        핵심표=("법령", "조문", "판례", "헌재결정례", "법령해석례"),
        # ⚠️ **원장은 일이 끝나면 줄어드는 표다.** 급감으로 세면 가장 잘 돌아간 날이 가장
        #    빨갛다 — 실측 `수집실패 507→1` 이 그 형태였다. `수집상태` 는 자료종류·단계마다
        #    한 행이라 급감할 일이 없어 넣지 않는다.
        # ⚠️ **`수집실패` 를 빼도 해로운 절반은 적재 등식이 잡는다** — '없음'·'본문없음'·
        #    '철회'로 확정된 행이 사라지면 합집합이 그만큼 줄어 빨개진다. 남는 절반
        #    ('재시도' 소실)은 다음 수집이 다시 시도해 스스로 메우므로 사고가 아니다.
        원장표=("수집실패",),
    ),
    "congress": 명세(
        이름="CONGRESS",
        스크립트=스킬 / "Scripts/congress/db.py",
        감사스크립트="Scripts/congress/audit.py",
        핵심표=("의원", "의안", "회의", "발언", "표결"),
    ),
}
...
def 급감표(코퍼스명: str, 전: dict[str, int],
    ...
    ⚠️ **원장을 목록에서 빼지 않고 표지만 단다.** 원장(`수집실패` 류)은 일이 끝나면
    줄어드는 표라 급감으로 세면 **가장 잘 돌아간 날이 가장 빨갛다** — 실측으로
    `수집실패 507→1` 이 급감 게이트를 빨갛게 만들었고 핵심표는 평평했다.
...
def 이행전_확인(코퍼스명: str, db경로, *, 시간: float = 24.0, 보관=None) -> 세대:
    ...
    if not g:
        raise SystemExit(
            ...
            f"   `uv run Scripts/backup.py {코퍼스명} --이행직전` 을 먼저 돌려라."
        )
    if not 신선한가(g, 시간):
        raise SystemExit(
