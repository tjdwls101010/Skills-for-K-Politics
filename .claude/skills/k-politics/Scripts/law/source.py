...
# 언제 막혀도 이상하지 않다 — `verify.py` V4 가 감시한다.
최대페이지크기 = 1000
...
class 자료종류:
    """DB 에 담는 네 종. CLI 도 문서도 이 한국어 이름만 쓴다.

    행정규칙·행정심판례·자치법규는 **DB 밖이다** — `direct.py` 가 실시간으로 부른다.
    그쪽 target 은 `live/commands.py` 의 `커맨드` 표가 갖고 있고, 그 둘이 갈려 있는 것이
    "이 DB 에 있나 없나"를 코드에서 읽을 수 있게 하는 경계다.
    """
...
class 봉투정보:
    ...
    def __init__(self, target, 목록봉투, 리스트키, 본문봉투, 식별자, 목록식별자=None):
        ...
        # ⚠️ 목록에서 그 식별자를 꺼낼 때 쓰는 **필드 이름**. 원천이 목록과 본문에 다른
        #    이름을 쓰는 일이 잦아 명시한다. 같다고 가정하면 None 을 ID 로 써서 본문
        #    조회가 **전량** 실패한다 — 목록 수집은 성공하므로 한참 뒤에야 드러난다.
        self.목록식별자 = 목록식별자


# ⚠️ **이 표를 규칙으로 대체하지 마라.** 값들을 실호출로 전수 재확인했고 `verify.py` 가
#    이 표를 기준값으로 쓴다.
봉투: dict[str, 봉투정보] = {
    ...
    # ⚠️ 원천이 서로 다른 target 에 같은 본문 봉투 이름을 쓴다.
...
목록기본: dict[str, dict[str, str]] = {
    # datSrcNm 을 안 걸면 169,938 건이 오고 그중 49,779 건은 본문 조회가 영구 실패한다.
    "판례": {"datSrcNm": "대법원"},
}
