...
def _직전정상(conn) -> tuple[str | None, list]:
    ...
    if (백업 := _백업모듈()) and (기준 := 백업.기준선(_코퍼스이름)):
        ...
        # ⚠️ **면제할 때도 비교 가능한 표는 재어 둔다.** 판정에서 빼는 것과 눈에서
        #    없애는 것은 다르다 — 그 창 안에서 진짜 대참사가 나면 게이트는 초록인데
        #    아무 흔적이 없다. 재어 둔 값은 보고값이 낸다.
        비교가능 = {t: v for t, v in 전.items() if t in 지금}
        답 = (((_스키마가갈림, 이름, 사라진,
                백업.급감표(_코퍼스이름, 비교가능, 지금)), []) if 사라진
              else (이름, 백업.급감표(_코퍼스이름, 전, 지금)))
    _직전캐시[id(conn)] = 답
    return 답
...
def _급감상세(conn) -> str:
    ...
    if isinstance(이름, tuple):
        ...
        _, 세대, 사라진, 본 = 이름
        줄 = (f"{세대} 는 스키마가 다른 코퍼스다 — 견주지 않았다"
             f" (그 세대에만 있는 표: {' · '.join(sorted(사라진))})."
             " 이 축은 **감사를 통과한** 새 스키마 세대가 생기면 되살아난다")
        if 본:
            # ⚠️ **면제했다고 눈에서도 없애면 안 된다.** 그 창 안에서 진짜 대참사가 나면
            #    게이트는 초록인데 아무 흔적이 없다. 판정에서만 빼고 관측은 남긴다.
            줄 += " · 그래도 줄어든 것: " + " · ".join(
                f"{t} {옛:,}→{새:,}" for t, 옛, 새, _ in 본)
        return 줄
    if not 난것:
...
비어도되는뷰: dict[str, tuple[str, str]] = {
    ...
    # ⚠️ **뷰가 거는 조건을 그대로 건다.** `부칙여부=0` 을 빠뜨리면 부칙 참조만 있는 DB 가
    #    "입력은 있는데 뷰가 비었다"로 잡혀 **정상 상태가 위반이 된다.**
    "조문판단": ("참조에서 현행 조 좌표를 하나도 못 풀었다",
              "SELECT 1 FROM 의율조문"
              " WHERE 법령ID IS NOT NULL AND 조 IS NOT NULL AND 부칙여부 = 0 LIMIT 1"),
...
게이트: list[tuple[str, str, str]] = [
    ...
    #    전량 실측에서 0 이다.
    # ⚠️ **`대상제목 IS NULL` 은 여기 넣지 마라.** 파서가 **일부러** 비우는 자리다 —
    #    한 근거문구가 두 법을 가리키면 고를 근거가 없어 NULL 이 맞는 답이고, 그건 원천의
    #    모호함이지 우리 잘못이 아니다. 게이트로 걸면 고칠 수 없는 빨간불이 되고,
    #    **고칠 수 없는 빨간불은 표 전체를 무시하게 만든다.** 그 수는 보고값이 본다.
    ("A22", "위임 파서가 자기 규칙을 어긴 행 (조 0 · 대상 조 0)",
...
보고: list[tuple[str, str, str]] = [
    ...
    ("R8", "그중 2020년 이후 판례 — 현행법을 인용하는 구간이라 늘면 파서부터 의심한다",
    ...
    ("R29", "대상 이름을 못 정한 위임 — 한 근거문구가 두 법을 가리켜 고를 근거가 없는 것이다",
    ...
    ("R19", "위임을 받았는데 0행인 법령 — 시행령·시행규칙에 안 미뤘거나 원천이 안 준 것이다",
