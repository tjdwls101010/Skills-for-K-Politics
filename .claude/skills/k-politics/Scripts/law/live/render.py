...
# ── 행정규칙 (DB 밖이다) ─────────────────────────────────────
#
# ⚠️ **행정규칙은 이 DB 에 없다.** 본문 326MB 의 98%가 정무위 밖 부처라 담지 않기로 했고,
#    감독규정처럼 실제로 필요한 것은 여기서 실시간으로 본다. 그래서 이 둘은 "DB 에 있는
#    자료의 커맨드는 여기 없다"는 원칙의 예외가 아니라 **그 원칙이 가리키는 바깥**이다.
...
def 렌더_행정규칙검색(payload):
    ...
    for 행 in 행들:
        줄들.append(" ".join(str(v or "-") for v in (
            행.get("행정규칙명"), 행.get("행정규칙종류"), 행.get("소관부처명"),
            정규화.날짜(행.get("발령일자")), 정규화.날짜(행.get("시행일자")),
            행.get("현행연혁구분"), 행.get("행정규칙일련번호"))))
    return "\n".join(줄들) if 행들 else "0건"


def 렌더_행정규칙본문(payload):
    """⚠️ **`조문내용` 의 타입이 두 가지다.** `조문형식여부='N'`(실측 10%)이면 배열이
    ...
    머리 = [
        ...
        f" 제{기본.get('발령번호', '-')}호 · 시행 {정규화.날짜(기본.get('시행일자')) or '-'}"
        f" · {기본.get('현행여부', '-')}",
        "",
    ]
    return "\n".join(머리 + [_내용줄(payload.get("조문내용"))]
                    + ([""] + [_내용줄(payload.get("부칙"))] if payload.get("부칙") else []))
